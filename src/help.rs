...
pub static HELP_SECTIONS: &[HelpSection] = &[
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: "m{a-z}", description: "Set bookmark at cursor (e.g. ma)" },
            HelpEntry { key: "`{a-z} / '{a-z}", description: "Jump to bookmark (pushed onto jumplist; clamped to EOF after edits)" },
            HelpEntry { key: "] / [", description: "Jump to next / previous diff region (with :diff active, wraps around)" },
        ],
    },
    HelpSection {
        id: "editing",
        ...
        entries: &[
            ...
            HelpEntry { key: "Ctrl+P", description: "Overwrite paste (no file growth, clamp to EOF)" },
            HelpEntry { key: ".", description: "Repeat last change (with count)" },
            HelpEntry { key: "q{a-z}", description: "Record macro into register (keys execute while recorded); q stops" },
            HelpEntry { key: "@{a-z} / @@", description: "Play macro / repeat last played macro (count prefix: 3@a)" },
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: "Ctrl+V", description: "Enter Visual Block mode (rectangular selection)" },
            HelpEntry { key: "v / V / Ctrl+V", description: "Switch between char/line/block selection (anchor unchanged)" },
            ...
            HelpEntry { key: "i", description: "Block insert at left edge (block mode only)" },
            HelpEntry { key: "a", description: "Block append at right edge (block mode only)" },
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: ":w <path> +L200", description: "Extract 200 bytes from cursor (also +200 / +0xA0); <start> <end>, <start> +L<len>, $ (to EOF), and mark refs 'a / 'a,'b supported" },
            ...
            HelpEntry { key: ":block", description: "Enter visual block mode (alternative to Ctrl+V)" },
            HelpEntry { key: ":overpaste", description: "Overwrite paste (alternative to Ctrl+P); alias :op" },
            HelpEntry { key: ":marks", description: "List all bookmarks (register:offset)" },
            HelpEntry { key: ":delmarks a b / :delmarks!", description: "Delete given bookmarks / all bookmarks" },
            HelpEntry { key: ":diff <path>", description: "Byte-compare buffer with a file: differences highlighted, ] / [ to navigate; run again to clear" },
            HelpEntry { key: "", description: "Diff compares the common length (length mismatch noted); snapshot taken at compare time" },
            HelpEntry { key: "", description: "Frame mode: differing bytes highlighted, frames containing diffs marked with red ! in the header" },
