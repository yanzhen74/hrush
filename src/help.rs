...
pub static HELP_SECTIONS: &[HelpSection] = &[
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: ":", description: "Enter Command mode; checksum commands apply to the selection" },
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: "L", description: "Open match list overlay (browse and jump to any match)" },
            HelpEntry { key: "j/k, Enter, q/Esc", description: "Browse matches, jump to selected match, close the list" },
            HelpEntry { key: "", description: "List shows +0xXXXX gap to the next match per row (last row: --)" },
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: ":sum", description: "Open checksum overlay (CRC16/CCITT-FALSE, CRC32/IEEE, MD5, SHA256, SUM8/16/32)" },
            HelpEntry { key: ":list", description: "Open search match list overlay (browse all matches, Enter to jump; shows +0x gap to next match)" },
            HelpEntry { key: ":matches", description: "Alias of :list" },
            HelpEntry { key: ":crc16 [preset]", description: "CRC16 checksum; presets: ccitt-false (default), xmodem, modbus, arc" },
            HelpEntry { key: ":crc32 [preset]", description: "CRC32 checksum; presets: ieee (default), c, stm32" },
            HelpEntry { key: "", description: "Custom CRC: :crc16/:crc32 take key=value overrides, optionally on a preset base" },
            HelpEntry { key: "", description: "Keys: poly= init= refin= refout= xorout= (0x hex or decimal; true/false)" },
            HelpEntry { key: "", description: "e.g. :crc32 ieee xorout=0 or :crc16 poly=0x8005 init=0xFFFF refin=true refout=true" },
            HelpEntry { key: ":md5", description: "MD5 digest of selection (or whole file)" },
            HelpEntry { key: ":sha256", description: "SHA-256 digest of selection (or whole file)" },
    ...
    HelpSection {
        ...
        entries: &[
            ...
            HelpEntry { key: "", description: "Status bar shows current frame number and length (len=0x...)" },
        ],
    },
];
